...
// Common copy functions for object_msgs/Emotion
#ifdef _MSC_VER
#pragma warning(push)
...
namespace Emotion {
  ...
  MDArray_T get_arr_roi(MDFactory_T& factory, object_msgs::msg::Emotion::_roi_type& val);
  //----------------------------------------------------------------------------
  void copy_from_arr_roi(sensor_msgs::msg::RegionOfInterest& val, const matlab::data::StructArray& arr) {
    // roi.x_offset
    try {
  ...
  //----------------------------------------------------------------------------
  OBJECT_MSGS_EXPORT void copy_from_arr(std::unique_ptr<object_msgs::msg::Emotion>& msg, const matlab::data::StructArray arr) {
    try {
        //emotion
        const matlab::data::CharArray emotion_arr = arr[0]["emotion"];
        msg->emotion = emotion_arr.toAscii();
    } catch (matlab::data::InvalidFieldNameException&) {
  ...
  //----------------------------------------------------------------------------
  OBJECT_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const std::shared_ptr<object_msgs::msg::Emotion>& msg) {
    auto outArray = factory.createStructArray({1,1},{"emotion","roi"});
    // emotion
    outArray[0]["emotion"] = factory.createCharArray(msg->emotion);
    // roi
