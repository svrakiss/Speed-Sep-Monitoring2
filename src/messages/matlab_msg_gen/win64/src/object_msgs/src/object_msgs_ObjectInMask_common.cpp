...
// Common copy functions for object_msgs/ObjectInMask
#ifdef _MSC_VER
#pragma warning(push)
...
namespace ObjectInMask {
  void copy_from_arr_roi(sensor_msgs::msg::RegionOfInterest& val, const matlab::data::StructArray& arr);
  MDArray_T get_arr_roi(MDFactory_T& factory, object_msgs::msg::ObjectInMask::_roi_type& val);
  //----------------------------------------------------------------------------
  void copy_from_arr_roi(sensor_msgs::msg::RegionOfInterest& val, const matlab::data::StructArray& arr) {
    // roi.x_offset
    try {
  ...
  //----------------------------------------------------------------------------
  OBJECT_MSGS_EXPORT void copy_from_arr(std::unique_ptr<object_msgs::msg::ObjectInMask>& msg, const matlab::data::StructArray arr) {
    try {
        //object_name
    ...
    try {
        //mask_array
        const matlab::data::TypedArray<float> mask_array_arr = arr[0]["mask_array"];
        size_t nelem = mask_array_arr.getNumberOfElements();
        for (size_t idx=0; idx < nelem; ++idx){
        	msg->mask_array.push_back(mask_array_arr[idx]);
        }
    } catch (matlab::data::InvalidFieldNameException&) {
  ...
  //----------------------------------------------------------------------------
  OBJECT_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const std::shared_ptr<object_msgs::msg::ObjectInMask>& msg) {
    auto outArray = factory.createStructArray({1,1},{"object_name","probability","roi","mask_array"});
    // object_name
    ...
    // mask_array
    outArray[0]["mask_array"] = factory.createArray<object_msgs::msg::ObjectInMask::_mask_array_type::iterator, float>({1, msg->mask_array.size()}, msg->mask_array.begin(), msg->mask_array.end());
    return outArray;
  }
} //namespace ObjectInMask
} //namespace matlabhelper
