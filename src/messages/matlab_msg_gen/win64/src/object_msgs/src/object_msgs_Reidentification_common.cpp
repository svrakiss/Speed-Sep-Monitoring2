...
// Common copy functions for object_msgs/Reidentification
#ifdef _MSC_VER
#pragma warning(push)
...
namespace Reidentification {
  ...
  MDArray_T get_arr_roi(MDFactory_T& factory, object_msgs::msg::Reidentification::_roi_type& val);
  //----------------------------------------------------------------------------
  void copy_from_arr_roi(sensor_msgs::msg::RegionOfInterest& val, const matlab::data::StructArray& arr) {
    // roi.x_offset
    try {
  ...
  //----------------------------------------------------------------------------
  OBJECT_MSGS_EXPORT void copy_from_arr(std::unique_ptr<object_msgs::msg::Reidentification>& msg, const matlab::data::StructArray arr) {
    try {
        //identity
        const matlab::data::CharArray identity_arr = arr[0]["identity"];
  ...
  //----------------------------------------------------------------------------
  OBJECT_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const std::shared_ptr<object_msgs::msg::Reidentification>& msg) {
    auto outArray = factory.createStructArray({1,1},{"identity","roi"});
    // identity
    outArray[0]["identity"] = factory.createCharArray(msg->identity);
    // roi
