...
// Common copy functions for object_msgs/PersonAttribute
#ifdef _MSC_VER
#pragma warning(push)
...
namespace PersonAttribute {
  ...
  MDArray_T get_arr_roi(MDFactory_T& factory, object_msgs::msg::PersonAttribute::_roi_type& val);
  //----------------------------------------------------------------------------
  void copy_from_arr_roi(sensor_msgs::msg::RegionOfInterest& val, const matlab::data::StructArray& arr) {
    // roi.x_offset
    try {
  ...
  //----------------------------------------------------------------------------
  OBJECT_MSGS_EXPORT void copy_from_arr(std::unique_ptr<object_msgs::msg::PersonAttribute>& msg, const matlab::data::StructArray arr) {
    try {
        //attribute
        const matlab::data::CharArray attribute_arr = arr[0]["attribute"];
        msg->attribute = attribute_arr.toAscii();
    } catch (matlab::data::InvalidFieldNameException&) {
  ...
  //----------------------------------------------------------------------------
  OBJECT_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const std::shared_ptr<object_msgs::msg::PersonAttribute>& msg) {
    auto outArray = factory.createStructArray({1,1},{"attribute","roi"});
    // attribute
    outArray[0]["attribute"] = factory.createCharArray(msg->attribute);
    // roi
