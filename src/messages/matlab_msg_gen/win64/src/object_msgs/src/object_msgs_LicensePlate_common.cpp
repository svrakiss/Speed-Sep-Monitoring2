...
// Common copy functions for object_msgs/LicensePlate
#ifdef _MSC_VER
#pragma warning(push)
...
namespace LicensePlate {
  ...
  MDArray_T get_arr_roi(MDFactory_T& factory, object_msgs::msg::LicensePlate::_roi_type& val);
  //----------------------------------------------------------------------------
  void copy_from_arr_roi(sensor_msgs::msg::RegionOfInterest& val, const matlab::data::StructArray& arr) {
    // roi.x_offset
    try {
  ...
  //----------------------------------------------------------------------------
  OBJECT_MSGS_EXPORT void copy_from_arr(std::unique_ptr<object_msgs::msg::LicensePlate>& msg, const matlab::data::StructArray arr) {
    try {
        //license
        const matlab::data::CharArray license_arr = arr[0]["license"];
        msg->license = license_arr.toAscii();
    } catch (matlab::data::InvalidFieldNameException&) {
  ...
  //----------------------------------------------------------------------------
  OBJECT_MSGS_EXPORT MDArray_T get_arr(MDFactory_T& factory, const std::shared_ptr<object_msgs::msg::LicensePlate>& msg) {
    auto outArray = factory.createStructArray({1,1},{"license","roi"});
    // license
    outArray[0]["license"] = factory.createCharArray(msg->license);
    // roi
